...
class KodbarsController extends Controller
{
	...
	public $listing_cols = ['id', 'kode_barang', 'nama_barang', 'tanggal_masuk', 'harga_beli', 'harga_jual', 'kategori_barang'];
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Kodbars", "create")) {
			...
			$result = mysqli_query($mysqli, "UPDATE kodbars as a, KATEGORIS as b SET a.harga_JUAL=a.harga_beli+a.harga_beli*b.mark_up/100 WHERE a.id='$insert_id' AND a.kategori_barang = b.id");
			$laporan = mysqli_query($mysqli, "INSERT INTO `laporans`(`id`, `deleted_at`, `created_at`, `updated_at`, `kode_barang`, `jumlah_penjualan`, `total_harga`) VALUES ('$insert_id',NULL,NULL,NULL,'$insert_id',0,0)");
	...
	public function destroy($id)
	{
		if(Module::hasAccess("Kodbars", "delete")) {
			...
			$mysqli = mysqli_connect('localhost', 'root', '', 'istock');
			$result = mysqli_query($mysqli, "DELETE FROM penjualans a, kodbars b WHERE a.kode_barang = b.kode_barang");		
